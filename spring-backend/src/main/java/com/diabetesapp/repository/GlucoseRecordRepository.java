...
public interface GlucoseRecordRepository extends JpaRepository<GlucoseRecord, Long> {
    ...
    @Query("SELECT AVG(g.value) FROM GlucoseRecord g WHERE g.user.id = :userId AND g.recordedAt >= :from")
    Optional<Double> findAverageByUserIdSince(@Param("userId") Long userId,
                                               @Param("from") LocalDateTime from);

    @Query("SELECT COUNT(g) FROM GlucoseRecord g WHERE g.user.id = :userId " +
           "AND g.recordedAt >= :from AND g.value BETWEEN :low AND :high")
    Long countInRangeByUserIdSince(@Param("userId") Long userId,
                                    @Param("from") LocalDateTime from,
                                    @Param("low") int low,
                                    @Param("high") int high);
